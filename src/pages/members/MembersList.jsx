...
export default function MembersList() {
  ...
  const canViewMemberDetails = isAdmin || isStaff;
  const notifyError = useNotifyError();
  const toast = useToast();

  // const { isStaff, canManageFinances } = useAuth();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      let query = supabase
        .from('members')
        .select('id, membership_no, full_name, phone, email, status, group_id, profile_id, welfare_groups(name), safe_profiles:profiles!members_profile_id_fkey(avatar_url)')
        .order('full_name');
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "email", header: "Email", render: (m) => m.email || "—" },
          {
            key: "group",
            header: "Group",
            render: (m) => m.welfare_groups?.name || "—",
          },
          {
            key: "status",
            header: "Status",
          ...
          },
          {
            ...
            render: (m) => {
              const canViewStatement =
                isStaff || (myMemberId && m.id === myMemberId);
