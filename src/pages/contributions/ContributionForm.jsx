...
export default function ContributionForm() {
  ...
  return (
    <>
      <Link to="/contributions" className="text-sm text-primary-900 hover:text-primary-700 inline-flex items-center gap-1 mb-4">
      ...
      <form onSubmit={handleSubmit} className="card-padded space-y-4 max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <select required className="input" value={form.member_id} onChange={(e) => update('member_id', e.target.value)}>
              ...
              {members.map((m) => (
                <option key={m.id} value={m.id}>{m.full_name} ({m.membership_no})</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input required type="number" step="0.01" min="0" className="input" value={form.amount} onChange={(e) => update('amount', e.target.value)} />
          </div>
          <div>
            <label className="label">Date *</label>
            <input required type="date" className="input" value={form.contribution_date} onChange={(e) => update('contribution_date', e.target.value)} />
          </div>

          <div>
            <label className="label">Type *</label>
            <select className="input" value={form.contribution_type} onChange={(e) => update('contribution_type', e.target.value)}>
          ...
          <div>
            ...
            <select className="input" value={form.period_month || ''} onChange={(e) => update('period_month', e.target.value)}>
              <option value="">—</option>
          ...
          <div>
            ...
            <input type="number" className="input" value={form.period_year || ''} onChange={(e) => update('period_year', e.target.value)} />
          </div>

          <div>
            <label className="label">Reference / M-Pesa code</label>
            <input className="input" value={form.reference_no} onChange={(e) => update('reference_no', e.target.value)} placeholder="e.g. SHX1A23BCD" />
          </div>

          {(form.contribution_type === 'project' || form.contribution_type === 'pledge') && (
            <div>
          ...
          {form.contribution_type === 'pledge' && pledges.length > 0 && (
            <div className="md:col-span-2">
              ...
              <select className="input" value={form.pledge_id} onChange={(e) => update('pledge_id', e.target.value)}>
                <option value="">— None —</option>
                {pledges.map((p) => (
                  <option key={p.id} value={p.id}>{p.purpose} (paid {p.paid_amount}/{p.pledge_amount})</option>
                ))}
              </select>
            </div>
          )}
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Link to="/contributions" className="btn-secondary">Cancel</Link>
